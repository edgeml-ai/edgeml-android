...
class EdgeMLApiIntegrationTest {
    ...
    @Test
    fun `healthCheck sends GET to correct path`() = runBlocking {
    ...
    @Test
    fun `registerDevice sends correct request body`() = runBlocking {
    ...
    @Test
    fun `registerDevice includes auth headers`() = runBlocking {
        val responseBody = json.encodeToString(
            DeviceRegistrationResponse(
                id = "uuid-1", deviceIdentifier = "d1", orgId = "o1",
    ...
    @Test
    fun `sendHeartbeat sends to correct device path`() = runBlocking {
    ...
    @Test
    fun `getDeviceGroups sends to correct path`() = runBlocking {
    ...
    @Test
    fun `getModel sends to correct path`() = runBlocking {
        val body = json.encodeToString(
            ModelResponse("m1", "o1", "test-model", "tensorflow_lite", "classification", null, 3, "2024-01-01T00:00:00Z", "2024-01-01T00:00:00Z")
        )
        server.enqueue(MockResponse().setBody(body).setResponseCode(200))
    ...
    @Test
    fun `getModelVersion sends to correct path`() = runBlocking {
        val body = json.encodeToString(
            ModelVersionResponse("v1", "m1", "1.0.0", "published", "s3://path", "tensorflow_lite", "abc", 1000, null, null, "2024-01-01T00:00:00Z", "2024-01-01T00:00:00Z")
        )
        server.enqueue(MockResponse().setBody(body).setResponseCode(200))
    ...
    @Test
    fun `getModelDownloadUrl sends correct path and format query`() = runBlocking {
        val body = json.encodeToString(
    ...
    @Test
    fun `reportInferenceEvent sends to correct path`() = runBlocking {
    ...
    @Test
    fun `getDevicePolicy sends to correct path`() = runBlocking {
    ...
    @Test
    fun `API returns error response for 404`() = runBlocking {
    ...
    @Test
    fun `API returns error response for 500`() = runBlocking {
    ...
    @Test
    fun `reportTrainingEvent sends to correct path`() = runBlocking {
    ...
    @Test
    fun `submitGradients sends to correct path`() = runBlocking {
    ...
    @Test
    fun `getDeviceVersion sends correct path with device and model`() = runBlocking {
    ...
    @Test
    fun `getLatestVersion sends correct path`() = runBlocking {
        val body = json.encodeToString(
            ModelVersionResponse("v1", "m1", "2.0.0", "published", "s3://path", "tensorflow_lite", "abc", 1000, null, null, "2024-01-01T00:00:00Z", "2024-01-01T00:00:00Z")
        )
        server.enqueue(MockResponse().setBody(body).setResponseCode(200))
