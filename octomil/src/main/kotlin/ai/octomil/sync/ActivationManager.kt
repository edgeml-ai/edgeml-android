...
class ActivationManager(
     ...
     *
     * For [ActivationPolicy.NEXT_LAUNCH]: marks the new version as staged;
     * call [activatePending] on next init to complete activation.
     *
    ...
    fun activate(
        ...
        return when (policy) {
            ...
            ActivationPolicy.NEXT_LAUNCH -> {
                ...
                Timber.d("Staged $artifactId@$artifactVersion for next_launch activation")
                ActivationResult(
                    success = true,
                    previousVersion = previousVersion,
            ...
            ActivationPolicy.MANUAL -> {
                metadataStore.markPendingActivation(artifactId, artifactVersion)
                Timber.d("Staged $artifactId@$artifactVersion for manual activation")
                ActivationResult(
                    success = true,
                    previousVersion = previousVersion,
                    newVersion = metadataStore.entry(artifactId, artifactVersion)?.modelVersion,
