...
/**
 * Advertises an Octomil device on the local network via Android's NSD (Network Service Discovery).
 *
 * The CLI (`octomil deploy --phone`) scans for `_octomil._tcp.` services on the local network
...
class NsdAdvertiser(private val context: Context) {
    ...
    companion object {
        /** Bonjour/mDNS service type for Octomil device discovery. */
        const val SERVICE_TYPE = "_octomil._tcp."

        /** Prefix for the advertised service name. */
        const val SERVICE_NAME_PREFIX = "Octomil-"
