...
class DeepLinkHandlerTest {
    ...
    @Test
    fun `parse returns Pair action for valid octomil pair URI with token and host`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "pair",
            queryParams = mapOf("token" to "abc123", "host" to "https://api.octomil.com"),
        )
    ...
    fun `parse returns null when token parameter is missing`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "pair",
            queryParams = mapOf("host" to "https://api.octomil.com"),
        )
    ...
    fun `parse returns null when token parameter is blank`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "pair",
            queryParams = mapOf("token" to "", "host" to "https://api.octomil.com"),
        )
    ...
    @Test
    fun `parse returns Unknown for unrecognized octomil action`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "settings",
    ...
    @Test
    fun `parse returns Unknown for octomil scheme with different host`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "deploy",
    ...
    @Test
    fun `parse returns Unknown for octomil scheme with null host`() {
        val uri = mockUri(
            scheme = "octomil",
            host = null,
        )
    ...
    fun `handleIntent extracts URI from intent and parses valid pairing link`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "pair",
            queryParams = mapOf("token" to "intent-token", "host" to "https://intent.octomil.com"),
        )
        ...
        assertEquals("https://intent.octomil.com", action.host)
    }
    ...
    @Test
    fun `handleIntent returns null for non-octomil intent`() {
        val uri = mockUri(
            scheme = "https",
            host = "example.com",
        )
    ...
    @Test
    fun `handleIntent returns Unknown for unrecognized octomil intent`() {
        val uri = mockUri(
            scheme = "octomil",
            host = "unknown-action",
        )
    ...
    @Test
    fun `SCHEME constant is octomil`() {
        assertEquals("octomil", DeepLinkHandler.SCHEME)
    }

    @Test
    fun `DEFAULT_HOST constant points to production API`() {
        assertEquals("https://api.octomil.com", DeepLinkHandler.DEFAULT_HOST)
    }
    ...
    fun `MANIFEST_INTENT_FILTER contains required XML elements`() {
        ...
        assert(filter.contains("android:scheme=\"octomil\""))
