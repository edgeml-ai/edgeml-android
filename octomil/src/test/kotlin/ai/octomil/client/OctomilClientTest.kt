...
class OctomilClientTest {
    ...
    fun `builder creates client with all injected deps`() {
        val built = OctomilClient.Builder(context)
            .config(config)
            .ioDispatcher(testDispatcher)
            .mainDispatcher(testDispatcher)
            .api(api)
    ...
    fun `isInitialized returns true after build`() {
        assertTrue(OctomilClient.isInitialized())
    }
    ...
    fun `train fails when device not registered`() = runTest(testDispatcher) {
        setClientReady() // no device ID
        val dataProvider = mockk<ai.octomil.training.TrainingDataProvider>(relaxed = true)
